...
Use this way to run on Docker Desktop:
Docker Desktop GUI

Open Docker Desktop → Images tab
Find consultation-app → click the Run button (▶)
Click Optional settings to expand the form
Under Environment variables, add each one:
CLERK_JWKS_URL = https://shining-falcon-24.clerk.accounts.dev/.well-known/jwks.json
OPENAI_API_KEY = (your key from .env)
Under Ports, set Host port 8000 → Container port 8000
Click Run
